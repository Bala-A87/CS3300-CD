...
public class GJDepthFirst<R,A> implements GJVisitor<R,A> {
   ...
   //
    ...
    HashMap<String, HashMap<Integer, Integer>> allocatedRegisters;
    HashMap<String, HashMap<Integer, Integer>> spilledValues;
    int passedArgs;
    ...
    public GJDepthFirst() {
      ...
      allocatedRegisters = null;
      spilledValues = null;
      passedArgs = 0;
    }

    void allocateRegister(int tempNo) {
      if(allocatedRegisters.get(currScope).size() < REGISTERS.length && !allocatedRegisters.get(currScope).containsKey(tempNo))
         allocatedRegisters.get(currScope).put(tempNo, allocatedRegisters.get(currScope).size());
      else if(!allocatedRegisters.get(currScope).containsKey(tempNo) && !spilledValues.get(currScope).containsKey(tempNo)) {
         noSpilled++;
         spilledValues.get(currScope).put(tempNo, spilledValues.get(currScope).size());
      }
    }

    boolean checkIfArgument(int tempNo) {
      return tempNo < currNoOfArgs;
    }
...
//
   ...
   public R visit(BinOp n, A argu) {
      ...
      if(build) {
         ...
         // TEMP usedTemp = tempMap.get(currScope).get(tempNo);
         // currStmt.addUse(usedTemp);
         if(!checkIfArgument(tempNo)) allocateRegister(tempNo);
         n.f2.accept(this, argu);
      }
      else {
         n.f0.accept(this, argu);
